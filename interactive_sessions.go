...
func (a *app) startInteractiveSession() (*interactiveSession, error) {
	...
	return a.newInteractiveSession([]types.Message{{Role: "system", Content: a.systemPromptWithSkills()}})
}
...
func (a *app) initializeInteractiveSession(session *interactiveSession) error {
	...
	if session.id == "" {
		if session.messages == nil {
			session.messages = []types.Message{{Role: "system", Content: a.systemPromptWithSkills()}}
		}
		store, err := a.ensureSessionStore()
		if err != nil {
			return err
...
func (a *app) newInteractiveSession(messages []types.Message) (*interactiveSession, error) {
	store, err := a.ensureSessionStore()
	if err != nil {
		return nil, err
	}
...
func (a *app) switchToNewSession(session *interactiveSession) error {
	...
	next, err := a.newInteractiveSession([]types.Message{{Role: "system", Content: a.systemPromptWithSkills()}})
	if err != nil {
		return err
	}
	*session = *next
...
func deriveSessionTopic(messages []types.Message) string {
...
func latestDirectUserPrompt(messages []types.Message) string {
...
func (a *app) runGoal(ctx context.Context, session *interactiveSession, objective string) bool {
	...
	if objective != "" {
		...
		if err := a.saveInteractiveSession(session); err != nil {
			a.writeInteractiveSystem(fmt.Sprintf("[goal] incomplete: could not save checklist reset: %v", err))
			return false
		}
	} else {
	...
	for iteration := 1; iteration <= a.cfg.maxGoalIterations; iteration++ {
		...
		if err != nil {
			_ = a.saveInteractiveSession(session)
