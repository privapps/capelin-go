...
func TestServerIntakeParityThroughDeliveryModes(t *testing.T) {
	...
	expiresAt, ok := a.dataStore.Expiry(accepted["id"])
	if !ok || time.Until(expiresAt) < asyncResultTTL-time.Minute {
		t.Fatalf("async result TTL was not preserved: %v", expiresAt)
	}
}
